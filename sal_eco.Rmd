...
## Salmonella | All results as boxplot
...
All replicates from single repeats are the same.
...
filter(frac_biofilm_D, frac == 1, strength == "medium") %>% 
    group_by(strain, conditions) %>% 
    summarise(n = length(medium)) %>% 
    filter(n == 4)
...
filter(frac_biofilm_D, frac == 1, strength == "strong") %>% 
  group_by(strain, conditions) %>% 
  summarise(n = length(medium)) %>% 
  filter(n == 4) %>% 
  select(strain)
